...
def eval_model(args):
    ...
    # === New: Align special tokens and pad/eos for merged models, resize vocab if needed ===
    mm_use_im_start_end = getattr(model.config, "mm_use_im_start_end", False)
    mm_use_im_patch_token = getattr(model.config, "mm_use_im_patch_token", True)

    added = 0
    if mm_use_im_patch_token:
        added += tokenizer.add_tokens([DEFAULT_IMAGE_PATCH_TOKEN], special_tokens=True)
    if mm_use_im_start_end:
        added += tokenizer.add_tokens([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN], special_tokens=True)

    # If no pad_token, use eos as fallback; also sync to model.config
    if tokenizer.pad_token is None and tokenizer.eos_token is not None:
        tokenizer.pad_token = tokenizer.eos_token
    if getattr(model.config, "pad_token_id", None) is None and tokenizer.pad_token_id is not None:
        model.config.pad_token_id = tokenizer.pad_token_id

    # Resize token embeddings if vocab size changed or tokenizer doesn't match embedding size
    ...
    ans_dir = os.path.dirname(answers_file)
    if ans_dir:
    ...
    for line in tqdm(questions):
        ...
        if hasattr(model.config, "cache_shape"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-qlora", action="store_true", help="enable LoRA path")
    ...
    # Safety check: prevent using qlora with merged models
    assert not args.use_qlora, "Currently evaluating with merged models, please do not use --use-qlora true."
