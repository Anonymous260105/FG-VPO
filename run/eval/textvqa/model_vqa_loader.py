...
def get_num_image_tokens(image_tensor, model):
    ...
    Output: The number of tokens corresponding to the images after being processed by the vision tower (based on the first image)
    """
    with torch.no_grad():
        ...
        # Fallback: roughly estimate from the vision tower
...
def eval_model(args):
    ...
    # === Register LLaVA special tokens and alignment ===
    ...
    # Resize if vocabulary size changes or model embeddings do not match tokenizer
    ...
    for (input_ids, images, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        # ====== Calculate image token number and RoPE safe margin ======
        ...
        # ====== Generation: Only pass pixel Tensor, no list, no image_sizes ======
        ...
        # Only decode the new generated part
